...
class PresensiController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $latitudelapangan = $lok[0];
        $longitudelapangan = $lok[1];
        $lokasi = $request->lokasi;
        $lokasiuser = explode(",", $lokasi);
        $latitudeuser = $lokasiuser[0];
        $longitudeuser = $lokasiuser[1];

        $jarak = $this->distance($latitudelapangan, $longitudelapangan, $latitudeuser, $longitudeuser);
        ...
        if($radius > $lok_kantor->radius){
            echo "error|Maaf,Anda Diluar Radius, Jarak Anda " .$radius." Meter Dari Lapangan|";
        }else{
        if($cek > 0){
            $data_pulang = [
                'nik' => $nik,
            ...
            if($simpan){
                echo "success|Terima Kasih dan Selamat Bekerja|in";
                echo 0;
                Storage::put($file, $image_base64);
            }else{
    ...
    public function histori(){
        ...
        public function storeizin(Request $request)
        {
            ...
            if($simpan){
                return redirect('presensi/izin')->with(['success' => 'Data Berhasil Disimpam']);
            } else {
    ...
    if (isset($_POST['exportexcel'])) {
        ...
        header("Content-Disposition: attachment; filename=Laporan Presensi Karyawan Lapangan $time.xls");
        }
